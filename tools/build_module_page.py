...
<body>
    ...
    <div class="module-layout">
      ...
      <article class="module-content">
        ...
        <section class="quiz-panel" aria-labelledby="treino-final">
          ...
          <p>As questões abaixo servem para revisar o conteúdo. A correção das objetivas acontece no próprio navegador. Depois da correção, o botão de envio abre um Google Forms pré-preenchido; o envio só acontece se o estudante conferir os dados e clicar em enviar no próprio formulário.</p>
          <form id="moduleQuiz">
  ...
  <script>
    ...
    document.getElementById("moduleQuiz").addEventListener("submit", (event) => {{
      ...
      latestModulePayload = buildSubmissionPayload(score, feedback, reportText);
    ...
    document.getElementById("sendTrainingReport").addEventListener("click", () => {{
      ...
      if (!latestModulePayload) {{
        status.textContent = "Corrija o treino antes de abrir o formulário de envio.";
        return;
      }}

      const formUrl = buildPrefilledGoogleFormUrl(latestModulePayload);
      if (!formUrl) {{
        status.innerHTML = "Google Forms ainda não configurado. Use Baixar TXT e envie manualmente para <a href=\\"mailto:mario.bastos.adv@gmail.com\\">mario.bastos.adv@gmail.com</a>.";
        return;
      }}

      window.open(formUrl, "_blank", "noopener");
      status.textContent = "Google Forms aberto em nova aba. Confira os dados e clique em enviar no próprio formulário.";
    }});
