...
AI_NOTICE = (
    "Este material fez uso de Inteligência Artificial Generativa (ChatGPT, modelo GPT-5.5, e Claude Anthropic, "
    "modelo Alps 5.0, conforme registro interno do projeto) para organização e estruturação do texto, revisão de "
    "clareza, adaptação didática e apoio à elaboração de questões e atividades. Foram observadas as diretrizes de "
    "integridade científica da Portaria CNPq nº 2.664/2026. A seleção do conteúdo, a conferência das fontes, as "
    "escolhas teóricas e a responsabilidade final pelo material são do docente responsável."
)
...
<body>
  ...
  <script>
    ...
    document.getElementById("moduleQuiz").addEventListener("submit", (event) => {{
      ...
      `).join("");
      document.getElementById("trainingReport").value = buildReport(score, feedback);
